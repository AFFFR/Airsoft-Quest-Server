...
const GM_CODE=process.env.GM_CODE || "firefox";
const games=new Map();
...
function newGame(){
 return {started:false,visibility:"discovery",objects:[],scores:{RED:0,BLUE:0},players:{}};
}
...
app.get("/debug-files",(_,res)=>{
  ...
  function walk(dir,base="") {
    try { return fs.readdirSync(dir,{withFileTypes:true}).flatMap(e=>{ const rel=path.join(base,e.name); return e.isDirectory()?walk(path.join(dir,e.name),rel):[rel]; }); }
    catch(e){ return []; }
  }
  res.json({cwd:process.cwd(),dirname:__dirname,files:walk(__dirname).slice(0,500)});
});
...
io.on("connection",s=>{
 s.on("create_game",(d,cb)=>{
   ...
   const g=newGame(); games.set(code,g);
 ...
 s.on("add_object",(d,cb)=>{
   ...
   const o={id:crypto.randomUUID(),name:txt(d?.name||"Objetivo",80),desc:txt(d?.desc||"",1000),
     team:t,points:Math.max(0,Number(d?.points)||0),radius:Math.max(1,Number(d?.radius)||30),
     qr:txt(d?.qr||"",200),lat,lon,photo:typeof d?.photo==="string"?d.photo:null,
     captured:{RED:false,BLUE:false},discovered:{RED:false,BLUE:false},
 ...
 s.on("reset",()=>{
   const {gid,role}=s.data,g=games.get(gid);
   if(g&&role==="GM"){for(const o of g.objects){o.captured={RED:false,BLUE:false};o.discovered={RED:false,BLUE:false}}g.scores={RED:0,BLUE:0};g.started=false;emitState(gid);}
 });
 s.on("capture",(d,cb)=>{
