...
function newGame(){return{started:false,visibility:"discovery",objects:[],scores:{RED:0,BLUE:0},players:{}}}
function cleanGame(g,role,team){
 const players={};
 for(const p of Object.values(g.players))if(role==="GM"||p.team===team)players[p.id]={...p};
 return{started:g.started,visibility:g.visibility,objects:g.objects,scores:g.scores,players};
}
function emitState(gid){
 const g=games.get(gid);if(!g)return;
 for(const s of io.sockets.sockets.values())if(s.data.gid===gid)s.emit("state",cleanGame(g,s.data.role,s.data.team));
}
...
/* Explicit interface routes */
const playerIndex=path.join(PUBLIC,"player","index.html");
const gmIndex=path.join(PUBLIC,"gm","index.html");
app.get("/player",(_,res)=>res.sendFile(playerIndex));
app.get("/player/",(_,res)=>res.sendFile(playerIndex));
app.get("/player/index.html",(_,res)=>res.sendFile(playerIndex));
app.get("/gm",(_,res)=>res.sendFile(gmIndex));
app.get("/gm/",(_,res)=>res.sendFile(gmIndex));
app.get("/gm/index.html",(_,res)=>res.sendFile(gmIndex));
...
app.get("/qr",async(req,res)=>{
 ...
 }catch(e){console.error(e);res.status(500).send("QR error")}
});

io.on("connection",s=>{
 s.on("create_game",(d,cb)=>{
  const code=txt(d?.gameCode||"",30).trim().toUpperCase(),gm=txt(d?.gmCode||"",80);
  if(!code||gm!==GM_CODE)return cb?.({ok:false,error:"Código GM inválido."});
  games.set(code,newGame());s.data={gid:code,role:"GM",team:"GM"};
  cb?.({ok:true,gameCode:code});emitState(code);
 });
 s.on("gm_login",(d,cb)=>{
  const code=txt(d?.gameCode||"",30).trim().toUpperCase(),gm=txt(d?.gmCode||"",80);
  if(gm!==GM_CODE||!games.has(code))return cb?.({ok:false,error:"Código da partida ou GM inválido."});
  s.data={gid:code,role:"GM",team:"GM"};cb?.({ok:true});emitState(code);
 });
 s.on("player_join",(d,cb)=>{
  const code=txt(d?.gameCode||"",30).trim().toUpperCase(),team=["RED","BLUE"].includes(d?.team)?d.team:null;
  if(!games.has(code)||!team)return cb?.({ok:false,error:"Partida/equipa inválida."});
  const g=games.get(code),name=txt(d?.name||"Jogador",40).trim()||"Jogador";
  g.players[s.id]={id:s.id,name,team,lat:null,lon:null,accuracy:null};
  s.data={gid:code,role:"PLAYER",team};cb?.({ok:true});emitState(code);
 });
 s.on("gps",d=>{
  const{gid,role}=s.data,g=games.get(gid),p=g?.players[s.id],lat=Number(d?.lat),lon=Number(d?.lon);
  if(!g||!p||role!=="PLAYER"||!Number.isFinite(lat)||!Number.isFinite(lon)||Math.abs(lat)>90||Math.abs(lon)>180)return;
  p.lat=lat;p.lon=lon;p.accuracy=Number(d?.accuracy)||null;emitState(gid);
 });
 s.on("add_object",(d,cb)=>{
  ...
  const o={id:crypto.randomUUID(),name:txt(d?.name||"Objetivo",80),desc:txt(d?.desc||"",1000),team:t,
   points:Math.max(0,Number(d?.points)||0),radius:Math.max(1,Number(d?.radius)||30),qr:txt(d?.qr||"",200),lat,lon,
   photo:typeof d?.photo==="string"?d.photo:null,captured:{RED:false,BLUE:false},discovered:{RED:false,BLUE:false},
 ...
 s.on("reset",()=>{
  const{gid,role}=s.data,g=games.get(gid);
  if(g&&role==="GM"){
   for(const o of g.objects){o.captured={RED:false,BLUE:false};o.discovered={RED:false,BLUE:false}}
   g.scores={RED:0,BLUE:0};g.started=false;emitState(gid);
  }
 });
 s.on("capture",(d,cb)=>{
  ...
  if(!o||!allowed(o,team)||o.captured[team]||String(o.qr)!==String(d?.qr||"").trim())
   return cb?.({ok:false,error:"QR/objetivo inválido."});
  ...
  o.discovered[team]=true;o.captured[team]=true;g.scores[team]+=o.points;cb?.({ok:true});emitState(gid);
 });
 s.on("disconnect",()=>{
...
server.listen(PORT,"0.0.0.0",()=>console.log(`Airsoft Quest online: ${PORT}`));
