  ...
  if (map) {
    ...
    for (i = 0; i < state.objects.length; i++) {
      ...
      if (objMarkers[o.id]) {
        ...
        objMarkers[o.id].setPopupContent(pop);
      } else {
        objMarkers[o.id] = L.marker([o.lat, o.lon]).addTo(map).bindPopup(pop);
      }
    }
    ...
    for (i = 0; i < ps.length; i++) {
      ...
      if (playerMarkers[p.id]) {
        ...
        playerMarkers[p.id].setPopupContent(pop);
      } else {
        playerMarkers[p.id] = L.circleMarker([p.lat, p.lon]).addTo(map).bindPopup(pop);
      }
    }
  }
}
...
$("create").onclick = function() {
  ...
  send("create_game", { gmCode: gmCode }, function(r) {
